...
class MemberEvent extends AbstractEvent
{
    public function handle_event(string $a_event, array $a_params): bool
    {
        ...
        if (isset($a_params['role_id']) && strpos($a_event, 'init_') === 0) {
        ...
        $lp_data = $eventDataAggregator->getLpStatusInfoByUsrAndObjId(
            (int) $a_params['usr_id'],
            (int) $a_params['obj_id']
        );
        $data['progress'] = $eventDataAggregator->getLpStatusRepresentation((int) ($lp_data['status'] ?? 0));
        if (is_array($lp_data)) {
